...
def prepare_pit(pdfs, zgrid, ztrue, code="", savefig=False):
    """Produce all the required rail objects and call the rail pit plot"""

    # pdfs
    # zgrid
    # ztrue
    ens_zspec = qp.Ensemble(qp.interp, data=dict(xvals=zgrid, yvals=pdfs))
    pitobj = PIT(ens_zspec, np.array(ztrue))  # PIT(fzdata, ztrue)
