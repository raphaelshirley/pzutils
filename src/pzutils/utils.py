...
def openLephare(filename):
    ...
    try:
        ...
        table=Table.read(filename,format='ascii.commented_header',
                      header_start=header_line,data_start=header_line+1,delimiter='\s')
        ...
        headers_len=len(lines[header_line].split())-1 #One hash at start needs to be removed
        line_nos=set()
        ...
        try:
            ...
            additional_headers=string_input+[
                'additional_input_col_{}'.format(i) for i in np.arange(np.max(list(line_nos))-(headers_len+len(string_input)))]
        except FileNotFoundError:
            additional_headers=['additional_input_col_{}'.format(i) for i in np.arange(np.max(list(line_nos))-headers_len)]
        
        lines_copy[header_line]=lines_copy[header_line][:-1]+" ".join(additional_headers)+"\n"
        ...
        for n,line in enumerate(lines_copy):
            ...
            if line_len!=np.max(list(line_nos)):
                ...
                lines_copy[n]=line[:-1]+" ".join(['-99' for i in np.arange(np.max(list(line_nos))-line_len)])+"\n"



        table=Table.read(lines_copy,format='ascii.commented_header',
              header_start=header_line,data_start=header_line+1,delimiter='\s')
...
def photoz_plots(z1,z1_name,z2,z2_name,
                 xlim=[0,4],ylim=[0,4],plot_del=False,name='',log=False):
    ...
    if plot_del:
        fig,((ax0,ax1),(ax2,ax3))=plt.subplots(nrows=2,ncols=2,sharey='row',sharex=True,figsize=(10,10))
    ...
    delz_lims=[-delz_extent/2,delz_extent/2]#[-(xlim[1]-xlim[0])/2,(xlim[1]-xlim[0])/2]
    mask2=z1>xlim[0]
    mask2&=z1<xlim[1]
    mask2&=z2>ylim[0]
    mask2&=z2<ylim[1]
    ...
    fig.suptitle("{}, {}/{} sources, $\eta$: {}%, $\sigma_{{NMAD}}$: {}".format(
        name,np.sum(mask),len(mask),round(outlier_frac*100,2),round(sigma_nmad,3)
    ))
    
    n_hex=30
    x_scale=np.max(z1[mask])/xlim[1]
    y_scale=np.max(z2[mask])/ylim[1]
    del_scale=(np.max(delz[mask2])-np.min(delz[mask2]))/(delz_lims[1]-delz_lims[0])
    ...
    hb0=ax0.hexbin(z1[mask],z2[mask],cmap=cmap,mincnt=1,linewidths=0.01,gridsize=gridsize)
    ...
    ax0.plot([0,xlim[1]],[0.05,1.05*xlim[1]],linestyle='-',color='red',linewidth='1.')
    ax0.plot([0,xlim[1]],[-0.05,0.95*xlim[1]],linestyle='-',color='red',linewidth='1.')
    ax0.plot([0,xlim[1]],[0.15,1.15*xlim[1]],linestyle='--',color='red',linewidth='1.')
    ax0.plot([0,xlim[1]],[-0.15,0.85*xlim[1]],linestyle='--',color='red',linewidth='1.')
    #divider0 = make_axes_locatable(ax0)
    #cax0 = divider0.append_axes("right", size="5%", pad=0.05)
    #cb=fig.colorbar(hb0,ax=ax0,cax=cax0)#,label='$N_{Objects}$')

    
    hb1=ax1.hexbin(z1[mask],z2[mask],bins='log',cmap=cmap,mincnt=1,linewidths=0.01,gridsize=gridsize)
    ...
    ax1.plot([0,xlim[1]],[0.05,1.05*xlim[1]],linestyle='-',color='red',linewidth='1.')
    ax1.plot([0,xlim[1]],[-0.05,0.95*xlim[1]],linestyle='-',color='red',linewidth='1.')
    ax1.plot([0,xlim[1]],[0.15,1.15*xlim[1]],linestyle='--',color='red',linewidth='1.')
    ax1.plot([0,xlim[1]],[-0.15,0.85*xlim[1]],linestyle='--',color='red',linewidth='1.')
    ...
    if plot_del:
        ...
        hb2=ax2.hexbin(z1[mask2],delz[mask2],
                       cmap=cmap,mincnt=1,linewidths=0.01,gridsize=del_gridsize)
        ...
        ax2.plot([0,xlim[1]],[0.05,0.05],linestyle='-',color='red',linewidth='1.')
        ax2.plot([0,xlim[1]],[-0.15,-0.15],linestyle='--',color='red',linewidth='1.')
        ax2.plot([0,xlim[1]],[0.15,0.15],linestyle='--',color='red',linewidth='1.')
        ...
        hb3=ax3.hexbin(z1[mask2],delz[mask2],
                       bins='log',cmap=cmap,mincnt=1,linewidths=0.01,gridsize=del_gridsize)
        ...
        ax3.plot([0,xlim[1]],[0.05,0.05],linestyle='-',color='red',linewidth='1.')
        ax3.plot([0,xlim[1]],[-0.15,-0.15],linestyle='--',color='red',linewidth='1.')
        ax3.plot([0,xlim[1]],[0.15,0.15],linestyle='--',color='red',linewidth='1.')
    ...
    fig_name='LePhare_{}_{}_{}'.format(z1_name,z2_name,name).replace('$','').replace('{','').replace('}','')
    fig.savefig('./figs/{}.png'.format(fig_name), 
                bbox_inches='tight')
    fig.savefig('./figs/{}.pdf'.format(fig_name), 
                bbox_inches='tight')
    return None

def photoz_plots_onecol(z1,z1_name,z2,z2_name,
                 xlim=[0,4],ylim=[0,4],delz_extent=0.4,name='',log=False,savefig=False,ticks0=[1,10],ticks1=[1,10]):
    """Standard photoz compariosn plots
    ...
    fig,((ax0),(ax1))=plt.subplots(nrows=2,sharex=True,figsize=(5.4,10)) #, layout='constrained')
    ...
    y_scale_del=(np.max(delz[mask2])-np.min(delz[mask2]))/(delz_lims[1]-delz_lims[0])
    ...
    hb0=ax0.hexbin(z1[mask],z2[mask],cmap=cmap,mincnt=1,bins='log',linewidths=0.01,gridsize=gridsize)
    ax0.set(xlim=xlim,ylim=ylim)
    ...
    ax0.text(0.03, 0.97, textstr, transform=ax0.transAxes, fontsize=10,
    ...
    hb1=ax1.hexbin(z1[mask2],delz[mask2],
                   cmap=cmap,mincnt=1,bins='log',linewidths=0.01,gridsize=del_gridsize)
    ...
    fig_name='ZZplot_{}_{}_{}'.format(
        z1_name,z2_name,name
...
def row_to_gauss(row,n):
    ...
    def gauss(x):
        ...
        return np.exp(-(x-mean)**2/(2*sigma**2))*norm/(sigma*np.sqrt(2*np.pi))
    return gauss
...
def row_to_gmm(row,n=0):
    ...
    if n==0:
        #Number of gaussians
        n=np.max([0 if not c.startswith('GMM_sigmas') else int(c.split('_')[-1]) for c in row.colnames])+1
    def gmm(x):
        pdf=np.full(len(x),0.)
...
def outlier_min(zgrid,pdf,max=True):
    ...
    outlier_fracs=np.array([np.trapz(pdf[i_vals[n]:j_vals[n]],x=zgrid[i_vals[n]:j_vals[n]]) for n in np.arange(len(zgrid))])
    ...
    if max:
        ...
        n_close=np.sum(np.isclose(outlier_fracs,outlier_fracs[np.argmax(outlier_fracs)],atol=1e-3,rtol=1e-3))
        om=zgrid[np.isclose(outlier_fracs,outlier_fracs[np.argmax(outlier_fracs)],atol=1e-3,rtol=1e-3)][n_close//2]
    
    
    return om,outlier_fracs

def quantiles(zgrid,cdf,q_values=[(1-0.997)/2,(1-0.68)/2,.5,1-(1-0.68)/2,1-(1-0.997)/2]):
...
def make_posteriors(table,spec_col='SPECZ_REDSHIFT',id_col='id'):
    ...
    for row in table[m]:
        try:
            ...
            #break
            peak,median,z_om,cdf,norm,pit=np.nan,np.nan,np.nan,np.nan,np.nan,np.nan
